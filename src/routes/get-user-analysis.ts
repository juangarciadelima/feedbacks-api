...
export const getUserAnalysis = new Elysia({
  tags: ["Análises"],
  detail: {
    description: "Get an analysis of the feedbacks of a specific user",
  },
})
  .use(authentication)
  .get(
    ...
    async ({ query }) => {
      ...
      const verifyIfFavorableFeedback = (feedbacks: Feedbacks[]) => {
        const acceptanceObject: {
          favorable: Feedbacks[];
          nonFavorable: Feedbacks[];
        } = {
          favorable: [],
          nonFavorable: [],
        };
        ...
        feedbacks.forEach((feedback) => {
          ...
          if (average > 3) {
            acceptanceObject.favorable.push(feedback);
          } else {
            acceptanceObject.nonFavorable.push(feedback);
          }
        });

        return acceptanceObject;
      };
      ...
      return {
        ...
        favorableNonFavorableFeedbacks:
          verifyIfFavorableFeedback(feedbacksNumber),
      };
    },
    {
      query: t.Object({
